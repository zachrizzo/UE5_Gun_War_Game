...
#define RightDirection UKismetMathLibrary::GetRightVector(CurrentActorRotationC)
#define ForwardDirection UKismetMathLibrary::GetForwardVector(CurrentActorRotationC)
#define CharRotation CurrentActorRotationC
...
bool UCpp_CoverSystemV2::TryStartCoverSystemC(FHitResult& ReturnWall, int DebugIndex)
{
	...
	for (int i = 0; i <= 1; i++)
	{
		const FVector VelocityXY = FVector(CharC->GetVelocity().X, CharC->GetVelocity().Y, 0.0);
		const float OffsetByVelocity = UKismetMathLibrary::MapRangeClamped(VelocityXY.Length(), 50.0, 550.0, TConfigS.VelocityOffsetScale.X, TConfigS.VelocityOffsetScale.Y);
		...
		TStart = CharC->GetActorLocation() + (RightDirection * Dire) + (ForwardDirection * 25.0 * OffsetByVelocity);
		TEnd = TStart + (ForwardDirection * 2.0);

		LineValid = UKismetSystemLibrary::BoxTraceSingle(CharC, TStart, TEnd, FVector(22 * OffsetByVelocity, 1, CharC->GetCapsuleComponent()->GetScaledCapsuleHalfHeight() * 0.7), CharRotation, Channel, false,
			ToIgnore, TraceType, LineResult, true, FLinearColor::Yellow, FLinearColor::Green, 0.1);

		if (LineValid == false)
		{	ReturnWall = LineResult; 
			//GEngine->AddOnScreenDebugMessage(-1, 0.25, FColor::Yellow, TEXT("Condition Failed: Line Valid = false"));
			return false; 
		}
		ACharacter* AsCharacter = Cast<ACharacter>(LineResult.GetActor());
		if (IsValid(LineResult.GetActor()) == false)
		{	
			ReturnWall = LineResult;
			//GEngine->AddOnScreenDebugMessage(-1, 0.25, FColor::Orange, TEXT("Condition Failed: LineResult.GetActor = false"));
			return false;
		}
		if (LineResult.GetActor()->ActorHasTag(TConfigS.TagNameToIgnore) == true || IsValid(AsCharacter) == true)
		{ 
			ReturnWall = LineResult;
			//GEngine->AddOnScreenDebugMessage(-1, 0.25, FColor::Red, TEXT("Condition Failed: IsValid(Character) = false or Actor Have Tag to ignore"));
			return false; 
		}
	...
	UHelpfulFunctionsBPLibrary::FindNearPointToWallSurface(CharC, CharC, ValidPoints, LineResult, ForwardDirection, 7, true, 15.0, FVector(0, 0, 0), DebugIndex);
	...
	//GEngine->AddOnScreenDebugMessage(-1, 0.25, FColor::Purple, TEXT("Condition Failed: Capsule Dont Have A Room"));
	return false;
}
...
void UCpp_CoverSystemV2::MoveCharToWallC(bool& WallValid, FHitResult& ReturnWall,float MoveToWallSpeed, int DebugIndex)
{
	...
	if (ValidPoints == false)
	{
		CachedWallNormalC = UKismetMathLibrary::VInterpTo_Constant(CachedWallNormalC, ForwardDirection * -1, UGameplayStatics::GetWorldDeltaSeconds(this), 15.0);
	...
	TargetCapRot = FRotator(0, TargetCapRot.Yaw, 0);

	const FRotator ActuallCharRot = FRotator(0, CurrentActorRotationC.Yaw, 0);
	...
	FRotator DeltaRotation = UKismetMathLibrary::NormalizedDeltaRotator(TargetCapRot, ActuallCharRot) * UKismetMathLibrary::FClamp(DeltaMultiply * 0.5, 0.0, 1.0);
...
void UCpp_CoverSystemV2::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	const bool RotUpdated = SetCurrentActorRotation_Implementation();

	// ...
}

bool UCpp_CoverSystemV2::SetCurrentActorRotation_Implementation()
{
	if (CharC)
	{
		CurrentActorRotationC = CharC->GetMesh()->GetSocketRotation("root");
		CurrentActorRotationC = UKismetMathLibrary::ComposeRotators(CurrentActorRotationC, FRotator(0, 90, 0));
		return true;
	}
	return false;
}
